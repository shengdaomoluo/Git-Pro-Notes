...
为了使用 Git 的协作功能，我们还需要有远程的 Git 仓库。因此，与他人合作的最佳方法即是建立一个与各位合作者都有权利访问，且可以在那里推送和拉取资料的共用仓库。

一个远程仓库通常只是一个**裸仓库（bare repository）**——即一个没有当前工作目录的仓库。<br>简单地说，裸仓库就是项目目录内的`.git`子目录内容，不包含其他资料。

## 4.1 协议（Protocols）

Git 可以使用四种不同的协议来传输资料（数据，data）：
...
### 4.1.1 本地协议（Local Protocol）

本地协议是最基础的一种协议。该协议中的远程版本仓库就是同一主机上的另一个目录。

如果我们使用共享文件系统，那么我们就可以从本地版本库克隆（clone）、推送（push）以及拉取（pull）项目。可以使用版本库路径作为 URL 来克隆一个版本库或者增加一个远程（remote）到现有的项目中。例如，克隆一个本地版本库，可以执行如下的命令：
...
* 如果仅是指定路径（无`file://`），Git 会尝试使用**硬链接（hard link）**或直接复制所需要的文件。

  > **硬链接（Hard Link）**：是操作系统中一种特殊的文件链接方式，其核心是通过文件层级的引用实现多个文件名指向同一物理存储位置。
  >
  > Git 使用硬链接实现本地仓库与远程仓库的同步（通过`--reference`选项的`clone`命令），几乎实现零拷贝，大幅提升克隆速度。

* 如果指定路径`file://`，Git 会触发平时用于网路传输资料的进程，那样传输效率会更低。
  * 指定`file://`的主要目的是取得一个没有外部参考（extraneous references）或对象（object）的干净版本库副本。

要增加一个本地仓库（Local Repository）到现有的 Git 项目，可以执行如下的命令：

```shell
$ git remote add local_proj /srv/git/project.git
```

其中，`local_proj`是要添加的本地仓库的名称。执行了上述命令后，就可以通过新的远程仓库名`local_proj`从该远程仓库推送和拉取更新了。

#### 4.1.1.1 优点（The Pros）

基于文件的版本库的优点是简单，并且直接使用了现有的文件权限和网络访问权限。

#### 4.1.1.2 缺点（The Cons）

* 通常共享文件系统比较难以配置，不方便从多个位置访问。
* 如果使用的是共享挂载的文件系统，那么访问本地版本库的速度会比较慢。
* 该协议表格保护仓库避免意外的损坏。
...
* 在 Git1.6.6 版本之前只有一个方式可用，十分简单并且通常是只读模式的。<br>该版本的协议一般被称为**哑 HTTP 协议**。
* Git1.6.6 版本引入了一种新的、更智能的协议，让 Git 可以像通过 SSH 那样智能的协商和传输数据。<br>该版本的协议又被称为**智能 HTTP 协议**。

#### 4.1.2.1  智能HTTP协议

智能 HTTP 的运行方式和 SSH 及 Git 协议类似，只是运行在标准的 HTTP/S 端口（port）上并且可以使用各种 HTTP 验证机制。<br>这意味着，使用起来会比 SSH 协议简单的多，比如可以使用 HTTP 协议的用户名/密码授权，免去设置 SSH 公钥。

智能 HTTP 协议或许已经是最流行的使用 Git 的方式了:

* 它既支持像`git://`协议一样设置匿名服务；
* 也可以像 SSH 协议一样提供传输时的授权和加密。而且只用一个 URL 就可以都做到，省去了为不同的需要设置不同的 URL。<br>如果我们要推送到一个需要授权的服务器上，服务器会提示你输入用户名和密码。这与从服务器获取数据时也一样。

事实上，类似GitHub 的服务，我们在网页上看到的 URL（比如：[本笔记的仓库地址：https://github.com/shengdaomoluo/Git-Pro-Notes](https://github.com/shengdaomoluo/Git-Pro-Notes)），和我们在克隆、推送时使用的是一样的。

#### 4.1.2.2  哑（Dumb）HTTP 协议

如果服务器没有提供智能 HTTP 协议的服务，Git 客户端会尝试使用更简单的“哑”HTTP 协议。哑 HTTP 协议里 web 服务器仅把裸版本库当作普通文件来对待，提供文件服务。哑 HTTP 协议的优美之处在于设置起来简单。<br>基本上，只需要把一个裸版本库放在 HTTP 根目录，设置一个叫做`post-update`的挂钩就可以了。此时，只能访问 web 服务器上的版本库，就可以克隆这个版本库。下面是设置从 HTTP 访问版本库的方法：
...
Git 自带的`post-update`挂钩（hooks）会默认执行合适的命令（`git update-server-info`），来确保通过 HTTP 的获取和克隆操作正常工作。这条命令会在通过 SSH 向版本库推送之后被执行；他人就可以通过类似下面的命令来克隆：

```shell
$ git clone https://example.com/gitproject.git
```

这里我们用了 Apache 里设置了常用的路径`/var/www/htdocs`，不过我们可以使用任何静态 Web 服务器——只需要把裸版本库放到正确的目录下就可以。Git 的数据是以基本的**静态文件（static file）**形式提供的。

> **静态文件（static file）**：是指文件的内容在编写时已经确定，并且在每次请求时返回的内容都是相同的，不会根据用户的请求而改变。<br>它们通常存放在网站的根目录下或专门的静态资源文件夹中，由 Web 服务器直接提供给请求的客户端。这些文件包括 HTML、CSS、JavaScript、图片、音频和视频文件等。

通常的，会在可以提供读/写的智能 HTTP 服务和简单的只读的哑 HTTP 服务之间选一个。<br>极少会将二者混合提供服务。

#### 4.1.2.3  优点

这里只关注智能 HTTP 协议的优点：

不同的访问方式只需要一个 URL 以及服务器只在需要授权时提示输入授权信息，这两个简便性让终端用户使用 Git 变得非常简单。相比 SSH 协议，可以使用用户名/密码授权是一个很大的优势——用户不必在使用 Git 之前在本地生成 SSH 密钥对，并把公钥上传至服务器。<br>对于非资深用户或者系统上缺少 SSH 相关程序的使用者，HTTP 协议的可用性是主要的优势。与 SSH 协议类似，HTTP 协议也非常快和高效。

也可以在 HTTPS 协议上提供只读版本库的服务，这样在传输数据的时候可以对数据加密；或者，可以让客户端使用 SSL 证书。

另一个好处是 HTTPS 协议被广泛使用，一般的企业防火墙都会允许这些端口的数据通过。

#### 4.1.2.4  缺点

在一些服务器上，架设 HTTPS 协议的服务端会比 SSH 协议的服务端更为麻烦一些。除了这一点之外，用其他协议提供 Git 服务与智能 HTTP 协议相比就几乎没有优势了。

如果在 HTTP 协议上使用需要授权的推送（pushing），管理凭证（credential）会比使用 SSH 密钥认证麻烦一些。然而，我们可以选择使用凭证存储工具，比如 macOS 的 Keychain 或者 Windows 的凭证管理器。

> **Keychain Access**是 macOS 中的一个关键工具，用于管理和存储用户的密码及其他敏感信息。<br>它保存了各种加密密码，包括网站、FTP 服务器、SSH 账户等。此外，它还管理用户的公钥和私钥，私钥用于代码签名以发布应用程序，而公钥通常随证书一起分发。Keychain Access 还支持访问控制、iCloud 同步以及 Touch ID 和 FaceID 集成等功能。通过使用 Keychain，用户可以确保其敏感信息被安全地存储和管理。

### 4.1.3  SSH协议

架设 Git 服务器时，常用 SSH 协议作为传输协议。因为在大多数环境下服务器已经支持通过 SSH 访问——即使没有，也很容易架设。SSH 协议也是一个验证授权的网络协议；并且，因为其普遍性，架设和使用都很容易。

通过 SSH 协议克隆版本库，我们可以**指定一个`ssh://`的** URL：
...
在上面两种情况中，如果我们不指定可选的用户名，那么 Git 会使用当前登录的用户的名字。

#### 4.1.3.1  优势

使用 SSH 协议有很多优势：

* 首先，SSH 架设相对比较简单——SSH 守护进程（服务器，daemons）很常见，多数网络管理员都有使用它的经验，并且多数操作系统都包含了它及相关的管理工具。
* 其次，通过 SSH 协议访问是安全的——所有的传输数据都要经过授权和加密。
* 最后，与 HTTPS 协议、Git 协议和本地协议一样，SSH 协议很高效，在传输前也会尽量压缩数据。

#### 4.1.3.2  缺点

SSH 协议的缺点在于：**不支持匿名访问 Git 仓库**。如果使用 SSH，那么即便是读取数据，使用者也必须通过 SSH 访问主机。<br>如果我们要同时提供匿名只读访问和 SSH 协议，那么我们除了为自己架设 SSH 服务之外，还得架设一个可以让其他人访问的服务。

### 4.1.4  Git协议

Git 协议是包含在 Git 里的一个特殊的守护进程（daemon）；该协议监听一个特定端口（9418，该端口提供类似于 SSH 协议的服务），但是 Git 协议访问无需任何授权和加密。<br>要让版本库支持 Git 协议，我们必须创建一个`git-daemon-export-ok`文件——它是 Git 协议守护进程为这个版本库提供服务的必要条件——但是除此之外没有任何安全措施——要么任何人都可以克隆这个版本库，要么任何人都不能克隆它。<br> 由于没有授权机制，一旦开放推送操作，意味着网络上任何知道这个项目 URL 的人都可以向项目推送数据。不用说，极少会有人这么做。

#### 4.1.4.1  优势

目前，Git 协议是 Git 使用的网络传输协议里最快的。Git 协议使用与 SSH 相同的数据传输机制，但是省去了加密和授权的开销。

#### 4.1.4.2  缺点

Git 协议的缺点是缺乏授权机制。<br>把 Git 协议作为访问项目版本库的唯一手段是不可取的。<br>一般的做法里，会同时提供 SSH 或者 HTTPS 协议的访问服务，只让少数几个开发者有推送（写）权限，其他人通过`git://`访问只有读权限。

## 4.2  在服务器上搭建Git

现在，我们将讨论如何在自己的服务器（server）上搭建 Git 服务（service）来运行这些协议。

在开始架设 Git 服务器之前，需要把现有仓库导出（export）为裸仓库（bare repository）——即一个不包含当前目录的仓库。在通过`clone`命令来创建一个新的裸仓库，需要在`clone`命令后加上`--bare`选项。按照惯例，裸仓库的目录名以`.git`结尾，如下所示：

```shell
$ git clone --bare my_project.git
cloning into bare repository 'my_porject.git'…
done.
```

现在，我们的`my_porject.git`目录中应该有 Git 目录的副本了。
...
> 命令`cp -Rf my_project/.git my_project.git`用于在 Unix 或 Linux 系统上执行文件和目录的复制操作。
>
> > **`cp`**：用于复制文件和目录。
> >
> > 选项**`-R`**：是`--recursive`(recursive /rɪˈkɜː.sɪv/ 回归的；递归的)的缩写。这意味着该命令会递归地复制目录中的所有内容。
> >
> > 选项**`-f`**是`--force`的缩写。它强制执行复制操作，即使目标文件已经复制也会覆盖它。
> >
> > **`my_project/.git`**：这是源文件，表示复制`my_project`目录中的`.git`子目录。
> >
> > > 其中`.git`是一个隐藏目录，包含 Git 仓库的所有元数据和对象。
> >
> > **`my_project.git`**：这是目标路径，表示将`.git`目录复制到`my_project.git`。这会创建一个新目录`my_project.git`，其中包含`.git`目录的所有内容。
>
> 这个命令可以用于备份 Git 仓库中的元数据和所有历史记录到另一个位置。通过复制`.git`目录，实际上是复制了整个 Git 仓库的历史信息。

虽然配置文件中有若干不同，但是对于我们的目的来说，这两种方式是一样的。这两种命令都是只取出 Git 仓库自身，不要工作目录，然后特别为它单独创建一个目录。

### 4.2.1  把裸仓库放到服务器上

既然我们有了裸仓库的副本，接下来要做的就是把裸仓库放到服务器上并设置我们的协议。

假设一个域名为`git.example.com`的服务器已经架好，并可以通过 SSH 协议连接，我们想把所有的 Git 仓库放在`/srv/git`目录下。假设服务器上存在`/srv/git/`目录，我们可以通过以下命令复制该裸仓库来创建一个新仓库：

```shell
$ scp -r my_project.git user@git.example.com:/srv/git
```

> 以上命令是一个用于通过**SCP**(Secure Copy Protocol，安全复制协议)将本地目录递归地复制到远程服务器上的命令。
>
> > `scp`是用于在本地和远程主机之间传输文件或目录的命令。<br>它基于 SSH（Secure Shell），因此传输过程是加密的。

此时，其他可通过 SSH 读取此服务器上`/srv/git`目录的用户，可运行以下命令来克隆我们的仓库。

```shell
$ git clone user@git.example.com:/srv/git/my_project.git
```

如果一个用户，通过使用 SSH 连接到一个服务器，并且该用户对`/srv/git/my_project.git`目录拥有可写权限，那么他将自动拥有推送权限。

如果到该项目目录中运行`git init`命令，并加上`--shared`选项，那么 Git 会自动修改该仓库目录的组权限为可写。注意，运行该命令的过程中，不会摧毁任何提交、引用等内容。
...
> **`git init`**是 Git 中用于初始化一个新的 Git 仓库的命令。<br>它会在当前目录下创建一个`.git`目录，该目录包含版本控制所需的所有元数据和配置文件。<br>通过这个命令，当前目录就会被转换为一个 Git 仓库，可以用来追踪文件的变化。

由此可见，根据现有的 Git 仓库创建一个裸仓库，然后把它放在我们和协作者都有 SSH 访问权限的的服务器上是非常容易的。

值得注意的是：

* 只要在服务器上假如可以用 SSH 登录的账号；
* 把裸仓库放在大家都有读取权限的地方；

我就已经准备好了一切，无需更多。

有一点应该记住：**如果只是和几个人在一个私有项目上合作的话，仅仅是一个 SSH 服务器和裸仓库就足够了**。

#### 4.2.1.1  小型安装

如果设备较少或者只想在小型开发团队中尝试 Git，那么一切都很简单。**假设 Git 服务最复杂的地方在于用户管理**。如果仓库被赋予不同的管理权限——对特定的用户可读，而给另一部分用户读写权限，那么访问和许可安排就会比较困难。

#### 4.2.1.2  SSH连接

如果我们有一台所有开发者都可以用 SSH 连接的服务器，架设我们的第一个仓库就十分简单了，因为我们几乎什么都不用做。如果我们想在仓库上设置更为复杂的访问控制权限，只要使用服务器操作系统的普通的文件系统权限就行了。

如果需要团队里的每个人都有写入权限，又不能给每个人在服务器上建立账户，那么提供 SSH 连接就是唯一的选择了。我们假设用来共享的仓库服务器上已经安装了 SSH 服务，而且我们可以通过它来访问服务器。<br>有以下几个方法给团队成员提供访问权限：

* 给团队里的每个人创建账号，这种方法很直接但也很麻烦。
  * 很难为每个人运行一次`adduser`（或者`useradd`）并且设置临时密码。
* 在主机上建立一个“git”账户，让每个需要写入权限的人发送一个 SSH 公钥，然后将其假如 git 账户的`~/ .ssh/atthorized_keys`文件。
  * 这样一来，所有人都将通过‘git’账户访问主机。这一点也不会影响提交的数据——访问主机用的身份不会影响提交对象的提交者信息。
