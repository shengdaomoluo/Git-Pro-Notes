...
不同的访问方式只需要一个 URL 以及服务器只在需要授权时提示输入授权信息，这两个简便性让终端用户使用 Git 变得非常简单。相比 SSH 协议，可以使用用户名/密码授权是一个一个很大的优势——用户不必在使用 Git 之前在本地生成 SSH 密钥对，并把公钥上传至服务器。<br>对于非资深用户或者系统上缺少 SSH 相关程序的使用者，HTTP 协议的可用性是主要的优势。与 SSH 协议类似，HTTP 协议也非常快和高效。
